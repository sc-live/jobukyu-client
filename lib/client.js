...
var Client = function() {
  ...
  function defineError (job, cbA) {
    reportError = function (err, cbB) {
      if (!job.metadata.dataToSendWhenFailed.progress) {
        job.metadata.dataToSendWhenFailed.progress = {};
      }
      job.metadata.dataToSendWhenFailed.progress.data = err;
      request.put({url: jobQueueUrl + '/jobs/' + job._id + '/fail', body: {job: job}, json: true}, function (resErr) {
        var cb = cbA || cbB;
